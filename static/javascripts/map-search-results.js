(function() {
  ...
  var MapSearchResults = window.LR.MapSearchResults = function(options) {
    this.$el = $(options.el);
    this.$infoEl = $(options.infoEl);
    this.geoUrl = options.geoUrl;
    this.postcodeCentre = options.postcodeCentre;
    this.postcode4326 = options.postcode4326;
    this.init();
  };
  MapSearchResults.prototype = {
    init: function() {
    ...
    onMoveend: function() {
      ...
      var url = this.geoUrl + '/titles?near=' + encodeURIComponent(JSON.stringify(this.postcode4326))
      $.ajax({
        url: url,
        dataType: 'jsonp',
